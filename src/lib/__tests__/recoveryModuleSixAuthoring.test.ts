// §Content-authoring pass Batch 6 — Module 6 authoring + anti-templating guard,
// plus the NEW activity-choice check (the gap Batch 5 surfaced).
...
import { BUILDING_A_LIFE_THAT_WORKS_FIELDS } from "@/lib/recovery.buildingALifeThatWorks.authored";

const lessons = () => liveLessonsInModule("building-a-life-that-works");

describe("Module 6 'Building a Life That Works' is really authored, not templated", () => {
  ...
  it("ships as PUBLISHED overrides authored by Cathy, not baseline edits", () => {
    for (const f of BUILDING_A_LIFE_THAT_WORKS_FIELDS) {
...
describe("Module 6 decision activities offer real per-lesson choices", () => {
  ...
  it("no longer offers the four labels all ten lessons shared", () => {
    const shared = ["Make the call anyway", "Ask my case manager for help", "Break it into one 10-minute step", "Bring someone with me"];
    for (const l of lessons()) {
...
describe("Module 6 keeps the established voice and format", () => {
