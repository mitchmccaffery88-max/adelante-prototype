// §Content-authoring pass Batch 8 — Module 8 authoring + anti-templating guard,
...
import { BECOMING_SOMEONE_NEW_FIELDS } from "@/lib/recovery.becomingSomeoneNew.authored";

const lessons = () => liveLessonsInModule("becoming-someone-new");

describe("Module 8 'Becoming Someone New' is really authored, not templated", () => {
  ...
  it("ships as PUBLISHED overrides authored by Cathy, not baseline edits", () => {
    for (const f of BECOMING_SOMEONE_NEW_FIELDS) {
...
describe("Module 8 decision activities offer real per-lesson choices", () => {
  ...
  it("no longer offers the four labels all ten lessons shared", () => {
    const shared = ["Ask what my future self would do", "Do one small brave thing", "Write it down", "Tell someone my goal"];
    for (const l of lessons()) {
...
describe("Module 8 keeps the established voice and format", () => {
