...
export const AdelanteEHR = {
  ...
  /**
   * §Recovery start date — Part 2 gate.
   *
   * Identical policy to `screenerAccess` for a Part 2 instrument, minus the
   * author/actor exception (nobody "administers" a self-reported date):
   *   • patient → their own value, always;
   *   • advocate → `advocatePart2Access(linkId)` plus link scope;
   *   • staff → `canAccess(role, "screeners_sud", patient)`, i.e. the live
   *     ASCMI `sud_treatment` consent check.
   * No new consent category: the existing Part 2 framework covers this
   * exactly, and a second category would be a second thing to revoke.
   */
  recoveryStartDateAccess(
    patientId: string,
    viewer: ScreenerViewer = { kind: "staff" },
  ): { allowed: boolean; reason?: string } {
    if (viewer.kind === "system") return { allowed: true };
    if (viewer.kind === "patient")
      return viewer.patientId === patientId
        ? { allowed: true }
        : { allowed: false, reason: "A patient may only read their own recovery date." };
    if (viewer.kind === "advocate") {
      const link = advocateLinks.find((l) => l.id === viewer.linkId);
      if (!link || link.patientId !== patientId)
        return { allowed: false, reason: "No advocate connection to this patient." };
      return AdelanteEHR.advocatePart2Access(link.id).unmasked
        ? { allowed: true }
        : { allowed: false, reason: "42 CFR Part 2 content is masked for this advocate connection." };
    }
    const role = viewer.role ?? getActingRole();
    const patient = patients.find((x) => x.id === patientId);
    const gate = canAccess(role, "screeners_sud", patient);
    return gate.locked
      ? {
          allowed: false,
          reason: gate.reason ?? "42 CFR Part 2 consent required to view this date.",
        }
      : { allowed: true };
  },
  /** Throwing read — same contract as `getScreenerResult`. */
  getRecoveryStartDate(
    patientId: string,
    viewer: ScreenerViewer = { kind: "staff" },
  ): string | undefined {
    const gate = AdelanteEHR.recoveryStartDateAccess(patientId, viewer);
    if (!gate.allowed) throw new Part2AccessError(gate.reason ?? "Access denied.");
    return patients.find((x) => x.id === patientId)?.recoveryStartDate;
  },
  /** Non-throwing read for UI: the date, or an explained mask. */
  viewRecoveryStartDate(
    patientId: string,
    viewer: ScreenerViewer = { kind: "staff" },
  ): { masked: boolean; date?: string; reason?: string } {
    const gate = AdelanteEHR.recoveryStartDateAccess(patientId, viewer);
    if (!gate.allowed) return { masked: true, ...(gate.reason ? { reason: gate.reason } : {}) };
    const date = patients.find((x) => x.id === patientId)?.recoveryStartDate;
    return { masked: false, ...(date ? { date } : {}) };
  },
  /**
   * Patient-controlled write. `null`/undefined clears it. No justification is
   * ever required, and nothing about a slip changes it automatically — the
   * only caller is an explicit patient action. Audited through the existing
   * unified stream (category `clinical`), never with the date's own history
   * in a parallel log.
   */
  setRecoveryStartDate(
    patientId: string,
    date: string | null,
    actor: { kind: "patient" | "staff"; id?: string; role?: string } = { kind: "patient" },
  ) {
    const p = patients.find((x) => x.id === patientId);
    if (!p) return;
    const previous = p.recoveryStartDate;
    if (date) p.recoveryStartDate = date;
    else delete p.recoveryStartDate;
    appendAudit({
      category: "clinical",
      action: date ? "recovery_start_date_set" : "recovery_start_date_cleared",
      patientId,
      actorRole: actor.role ?? actor.kind,
      ...(actor.id ? { actorId: actor.id } : {}),
      detail: { previous: previous ?? null, next: date ?? null },
    });
    emit();
  },
  /**
